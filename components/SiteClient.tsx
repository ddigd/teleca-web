...
async function saveCollectionToDB(fm: any, isNew: boolean, editingId: string | null) {
  ...
  const collectionData = {
    ...
    updated_at: new Date().toISOString(), theme_primary: fm.themePrimary || "#7C3AED", theme_bg: fm.themeBg || "#1a1a2e",
  };
    ...
    <React.Fragment>
      ...
      <div style={{ background: `linear-gradient(145deg, ${tbg}, ${tbg}cc, #0f1626)`, padding: mob ? "32px 16px" : "48px 32px", backgroundImage: gridBg, backgroundSize: "4rem 4rem" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "grid", gridTemplateColumns: mob ? "1fr" : "1fr 1fr", gap: mob ? 24 : 48, alignItems: "center" }}>
          ...
          <AutoImg src={item.mainImage} padding="24px" noShadow style={{ height: mob ? 280 : 400, border: "none", background: "transparent" }} />
        </div>
      </div>
...
function EditorForm({ editingId, onDone, showToast }) {
  ...
  return (<div><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 8 }}><h2 style={{ fontSize: 18, fontWeight: 900 }}>{isNew ? "신규 등록" : "수정"}</h2><div style={{ display: "flex", gap: 8 }}><Btn v="ghost" size="sm" onClick={() => onDone(null)}><X size={14} /> 취소</Btn><Btn v="blue" size="sm" onClick={save}><Save size={14} /> 저장</Btn></div></div>
    ...
    <div style={sec}><div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}><span style={{ fontSize: 14, fontWeight: 900 }}>테마 컬러</span></div><div style={{ display: "flex", gap: 16, alignItems: "flex-end" }}><div><label style={{ display: "block", fontSize: 11, fontWeight: 700, marginBottom: 6 }}>메인 컬러</label><input type="color" value={fm.themePrimary || "#7C3AED"} onChange={e => u("themePrimary", e.target.value)} style={{ width: 50, height: 36, border: "2px solid #000", cursor: "pointer", padding: 0 }} /></div><div><label style={{ display: "block", fontSize: 11, fontWeight: 700, marginBottom: 6 }}>배경 컬러</label><input type="color" value={fm.themeBg || "#1a1a2e"} onChange={e => u("themeBg", e.target.value)} style={{ width: 50, height: 36, border: "2px solid #000", cursor: "pointer", padding: 0 }} /></div><div style={{ fontSize: 11, color: "#888", paddingBottom: 4 }}>메인: CTA/탭 · 배경: 상단 그라데이션</div></div></div>
